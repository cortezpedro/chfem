...
cudapcgFlag_t solve_RD(cudapcgSolver_t *solver, cudapcgVar_t *res_x){
    ...
    // Initialize x values in CPU
    if (!(solver->x0_hasBeenSet_flag))
      zeros(d,n);
    HANDLE_ERROR(cudaMemcpy(res_x,d,n*sizeof(cudapcgVar_t),cudaMemcpyDeviceToHost)); // d is used to store x0 in GPU
    ...
    d_ptr_dotres = solver->dotprod_precond_stream(model,r,kernels_stream);  // delta = r*(M^-1*r)
    ...
    if (solver->x0_hasBeenSet_flag){
      ...
      solver->Aprod_stream(model,d,r,-1.0,CUDAPCG_TRUE,kernels_stream);       // r += -(A*x) (using d to store initial guess for x)
      d_ptr_dotres = solver->dotprod_precond_stream(model,r,kernels_stream);  // delta = r*(M^-1*r)
    ...
    delta_old = delta;
    solver->applyPreConditioner_stream(model,r,r,0.0,d,kernels_stream);  // s = M^-1 * r + 0.0*r (d=s on first iteration)
    HANDLE_ERROR(cudaStreamSynchronize(kernels_stream));
    ...
    solver->Aprod_stream(model,d,r,-a,CUDAPCG_TRUE,kernels_stream);         // r += -a*(A*d)
    d_ptr_dotres = solver->dotprod_precond_stream(model,r,kernels_stream);  // delta = r*(M^-1*r)
    // x += a*d
    offset = 0;
    for (unsigned int i=0; i<nStreams; i++){
    ...
    while (isResidualAboveTol(delta,delta_0,solver->num_tol) && solver->iteration < solver->max_iterations){
        ...
        solver->iteration++;
        solver->applyPreConditioner_stream(model,r,d,delta/delta_old,d,kernels_stream);  // d = M^-1 * r +(delta/delta_old)*d
        HANDLE_ERROR(cudaStreamSynchronize(kernels_stream));
        offset = 0;
        for (unsigned int i=0; i<nStreams; i++){
          HANDLE_ERROR(cudaMemcpyAsync(&solver->h_d[offset],&d[offset],memtransfer_streamSize[i],cudaMemcpyDeviceToHost,memtransfer_streams[i]));
