...
cudapcgFlag_t solve_XRD(cudapcgSolver_t *solver, cudapcgVar_t *res_x){
    ...
    delta_0 = solver->dotprod_precond(model,r);  // delta = r*(M^-1*r)
    
    // Check if x0=[0] has already satisfied an absolute tolerance
    // This is a safety check. As we perform dimensionless residual evaluation, in
    ...
    if (solver->x0_hasBeenSet_flag){
      ...
      solver->Aprod(model,x,r,-1.0,CUDAPCG_TRUE);   // r += -(A*x)
      delta = solver->dotprod_precond(model,r);     // delta = r*(M^-1*r)
    ...
    delta_old = delta;
    solver->applyPreConditioner(model,r,r,0.0,d);  // s = M^-1 * r + 0.0*r (d=s on first iteration)

    // First iteration outside of while loop
    solver->iteration++;
    a = delta / solver->dotprod_Aprod(model,d);  // a = delta/(d*(A*d))
    sumVecIntoFirst(x,d,a,n);                    // x += a*d
    solver->Aprod(model,d,r,-a,CUDAPCG_TRUE);    // r += -a*(A*d)
    delta = solver->dotprod_precond(model,r);    // delta = r*(M^-1*r)
    ...
    while (isResidualAboveTol(delta,delta_0,solver->num_tol) && solver->iteration < solver->max_iterations){
        ...
        solver->iteration++;
        solver->applyPreConditioner(model,r,d,delta/delta_old,d);  // d = M^-1 * r +(delta/delta_old)*d
        a = delta / solver->dotprod_Aprod(model,d);                // a = delta/(d*(A*d))
        sumVecIntoFirst(x,d,a,n);                                  // x += a*d
        solver->Aprod(model,d,r,-a,CUDAPCG_TRUE);                  // r += -a*(A*d)
        delta_old = delta;
