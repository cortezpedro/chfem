'''
  INSTRUCTIONS:
   ~$ python (or python3) compile_test.py -h
'''
#---------------------------------------------------------------------------
import sys
import os
...
def printVersion():
  print('LCC - UFF')
  print('Python script to compile chfem_gpu test. Version 1.1 (2020-2022)')
#---------------------------------------------------------------------------
def printHelp():
  printVersion()
  print('----------------------------------------')
  print('~$ python3 compile_test.py [options]')
  print('----------------------------------------')
  ...
  print('\t\t-DCUDAPCG_BLOCKDIM_128 ----- (default)')
  print('\t\t-DCUDAPCG_BLOCKDIM_256')
  print('\t\t-DCUDAPCG_BLOCKDIM_512')
...
if __name__ == '__main__':
  ...
  o_files[o_files.index('main'+o_ext)] = 'test'+o_ext
  ...
  if isLinux:
    compiler_directives = [
      ...
      'nvcc -c test.c -lm'+input_string + extra_compiler_options,
    ...
    for f in c_files:
      if not('\\physical_phenomena' in f) and not('\\report' in f) and not('main' in f):
        compiler_directives.append('nvcc -c '+input_string+f+extra_compiler_options)
    compiler_directives.append('nvcc -c test.c -lm'+input_string+extra_compiler_options)
