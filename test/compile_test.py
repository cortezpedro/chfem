...
if __name__ == '__main__':
  ...
  if isLinux:
    compiler_directives = [
      ...
      'nvcc -c test.c -Xcompiler -fopenmp'+input_string + extra_compiler_options,
    ...
    for f in c_files:
      if not('\\physical_phenomena' in f) and not('\\report' in f) and not('main' in f):
        compiler_directives.append('nvcc -c '+f+'-Xcompiler \"-openmp\"'+input_string+extra_compiler_options)
    compiler_directives.append('nvcc -c test.c -Xcompiler \"-openmp\"'+input_string+extra_compiler_options)
